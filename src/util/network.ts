...
const web3 = new Web3();
const networks: { [chainId: number]: { web3: Web3; admin: { address: string }; relayer?: Relayer } } = {};
...
if (HARDHAT_RPC) {
    networks[ChainId.Hardhat] = (() => {
        ...
        return {
            web3,
            admin: web3.eth.accounts.privateKeyToAccount(PRIVATE_KEY),
        };
    })();
}
...
if (MUMBAI_RELAYER) {
    networks[ChainId.PolygonMumbai] = (() => {
        ...
        return { web3: new Web3(provider), relayer, admin: { address: MUMBAI_RELAYER } };
    })();
}
...
if (POLYGON_RELAYER) {
    networks[ChainId.Polygon] = (() => {
        ...
        return { web3: new Web3(provider), relayer, admin: { address: POLYGON_RELAYER } };
    })();
}
