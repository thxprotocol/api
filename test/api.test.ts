...
import { ASSET_POOL, REWARD_POLL, tokenContract, WITHDRAW_POLL } from '../src/util/network';
...
describe('GET /members/:address', () => {
    it('HTTP 404 if member is not found', (done) => {
...
describe('POST /withdrawals', () => {
    ...
    it('HTTP 302', async (done) => {
        ...
        const { call, nonce, sig } = await signMethod(voter, ASSET_POOL.abi, poolAddress, 'proposeWithdraw', [
...
describe('POST /withdrawals/:address/withdraw (proposed withdrawal)', () => {
    ...
    it('HTTP 302 and redirect to withdrawal', async (done) => {
        const { call, nonce, sig } = await signMethod(voter, WITHDRAW_POLL.abi, withdrawPollAddress, 'finalize', []);
